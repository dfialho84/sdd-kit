...
description: "Padrões de qualidade para criação de tasks de implementação no formato tasks.md. Tasks são organizadas por requisito funcional (um bloco por REQ), com granularidade de card de board (uma task por método, endpoint ou migration). Cada task tem checkbox, descrição, rastreabilidade e dependências. Use junto com interview-guide e tasks-example como régua de qualidade."
---
